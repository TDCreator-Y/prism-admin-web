...
## 模块入口
- 对外导出入口：[index.ts](../src/router/routes/index.ts)
- 生成主流程：[dynamic-routes.ts](../src/router/routes/dynamic-routes.ts)
- 菜单树处理：[menu-tree.ts](../src/router/routes/menu-tree.ts)
- 路由转换：[route-transform.ts](../src/router/routes/route-transform.ts)
- 缓存逻辑：[cache.ts](../src/router/routes/cache.ts)

## 生成流程
1. `generateDynamicRoutes()` 先等待远程 UMD 组件注册完成，再尝试从本地缓存恢复动态路由。
2. 缓存不可用时，通过 [menu-tree.ts](../src/router/routes/menu-tree.ts) 的 `getRootMenu()` 调用菜单服务获取根菜单数据。
3. 扁平菜单数据经过 `getMenuTree()` 重组为树形结构。
4. 树形菜单传入 [route-transform.ts](../src/router/routes/route-transform.ts) 的 `generateRoutes()`，生成中间态 `ElegantRoute`。
...
## 菜单数据来源
- 当前运行时菜单入口是 [menu-service.ts](../src/router/routes/menu-service.ts) 的 `fetchMenuData()`。
...
## 缓存策略
- 缓存键定义在 [cache.ts](../src/router/routes/cache.ts) 的 `DYNAMIC_ROUTES_CACHE`。
...
## 全局配置依赖
- 运行时配置由 [global-config.ts](../src/router/routes/global-config.ts) 统一管理。
- 动态路由依赖 `InternalCode`、`UserCode`、`Origin`、`DisplayName` 等配置项。
- 默认配置从 `window.uiGlobalConfig` 读取，缺省时使用项目内置兜底值。

## 与 IframePage 的关系
- 页面型动态路由最终都落到 [index.vue](../src/views/_builtin/iframe-page/index.vue)。
- 路由 props 中会携带 `url`、`routeId`、`functionId`、`type` 等字段。
- IframePage 再根据 `type` 和动态权限结果选择渲染 Webview 或 Vue 远程组件。
- 多实例显示与切换由 [teleport-manager.ts](../src/store/modules/teleport-manager.ts) 负责管理。

## 调试建议
- 菜单数据异常时，先检查 [menu-service.ts](../src/router/routes/menu-service.ts) 返回值结构是否满足 `MenusMain.Results`。
